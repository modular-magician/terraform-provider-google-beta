...
* `identity_sources` -
  ...
  * `WORKFORCE_IDENTITY_FEDERATION`: Use external identities set up on Google Cloud Workforce
    				     Identity Federation.
  Each value may be one of: `IDENTITY_SOURCE_UNSPECIFIED`, `WORKFORCE_IDENTITY_FEDERATION`.
...
* `method` -
  ...
  * `ENROLLED_SECOND_FACTORS`: User can use any enabled 2nd factor.
  Possible values are: `METHOD_UNSPECIFIED`, `LOGIN`, `SECURE_KEY`, `ENROLLED_SECOND_FACTORS`.
...
* `policy_type` -
  ...
  * `DEFAULT`: This policy acts as a default if no other reauth policy is set.
  Possible values are: `POLICY_TYPE_UNSPECIFIED`, `MINIMUM`, `DEFAULT`.
...
* `output_credentials` -
  ...
  * `RCTOKEN`: Propagate attributes in the RCToken of the form: "
               additional_claims": { "my_attribute": ["value1", "value2"] }
  Each value may be one of: `OUTPUT_CREDENTIALS_UNSPECIFIED`, `HEADER`, `JWT`, `RCTOKEN`.
