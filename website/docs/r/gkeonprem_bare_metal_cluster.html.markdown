...
* `kubelet_config` -
  (Optional)
  The kubelet CPU manager policy.
...
* `cpu_manager_policy` -
  ...
  Possible values are: `NONE`, `STATIC`.
...
* `cpu_cfs_quota_period` -
  (Optional)
  The CPU Completely Fair Scheduler (CFS) quota period value.
  ...
  Defaults to "100ms".
  Structure is [documented below](#nested_cpu_cfs_quota_period).

* `feature_gates` -
  (Optional)
  A map of feature names to bools that enable or disable
...
* `pod_pids_limit` -
  (Optional)
  The maximum number of PIDs in any pod.
...
<a name="nested_cpu_cfs_quota_period"></a>The `cpu_cfs_quota_period` block supports:
