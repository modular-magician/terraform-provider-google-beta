...
* `key_attachment` - (Required) The identifier for this object. Format specified above.
...
<a name="nested_logical_structure_regions"></a>The `regions` block supports:
...
<a name="nested_logical_structure_regions_regions_metros"></a>The `metros` block supports:
...
<a name="nested_logical_structure_regions_regions_metros_metros_facilities"></a>The `facilities` block supports:
...
<a name="nested_logical_structure_regions_regions_metros_metros_facilities_facilities_zones"></a>The `zones` block supports:
...
<a name="nested_configured_availability_sla"></a>The `availability_sla` block supports:

* `effective_sla` -
  (Optional)
  Which SLA this group supports. Options are the same as the
  intent.
  Possible values are: `PRODUCTION_CRITICAL`, `PRODUCTION_NON_CRITICAL`, `NO_SLA`, `EFFECTIVE_SLA_UNSPECIFIED`.
...
<a name="nested_configured_availability_sla_intended_sla_blockers"></a>The `intended_sla_blockers` block supports:

* `blocker_type` -
  (Optional)
  The category of an unmet SLA requirement.
  Possible values are: `BLOCKER_TYPE_UNSPECIFIED`, `NO_ATTACHMENTS`, `INCOMPATIBLE_REGIONS`, `INCOMPATIBLE_METROS`, `NO_ATTACHMENTS_IN_METRO_AND_ZONE`, `MISSING_GLOBAL_ROUTING`, `OTHER`.
