...
#
#     Source file: https://github.com/GoogleCloudPlatform/magic-modules/tree/main/mmv1/third_party/terraform/website/docs/r/bigquery_table.html.markdown
#
...
* `ignore_schema_changes` - (Optional)  A list of fields which would act non-authoritative for each column in schema.
    **NOTE:** Currently `dataPolicies` and `policyTags` fields are supported. Any `dataPolicies` or `policyTags` updated outside terraform will be ignored if this option is used.
