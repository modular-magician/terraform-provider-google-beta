...
resource "google_compute_interconnect" "example-interconnect" {
  ...
  location             = "https://www.googleapis.com/compute/v1/projects/${data.google_project.project.id}/global/interconnectLocations/iad-zone1-1"
...
* `requested_features` -
  ...
  Each value may be one of: `MACSEC`, `IF_MACSEC`, `CROSS_SITE_NETWORK`.
