...
* `params` -
  ...
  ~> **Warning:** `params` is deprecated and will be removed in a future major release. Use `json_params` instead.

* `json_params` -
  (Optional)
  Params needed to access the source in structured json format.
