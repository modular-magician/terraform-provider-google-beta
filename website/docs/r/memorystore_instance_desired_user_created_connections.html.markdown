...
To get more information about InstanceDesiredUserCreatedConnections, see:
...
<div class = "oics-button" style="float: right; margin: 0 0 -15px">
  <a href="https://console.cloud.google.com/cloudshell/open?cloudshell_git_repo=https%3A%2F%2Fgithub.com%2Fterraform-google-modules%2Fdocs-examples.git&cloudshell_image=gcr.io%2Fcloudshell-images%2Fcloudshell%3Alatest&cloudshell_print=.%2Fmotd&cloudshell_tutorial=.%2Ftutorial.md&cloudshell_working_dir=memorystore_instance_desired_user_created_connections&open_in_editor=main.tf" target="_blank">
    <img alt="Open in Cloud Shell" src="//gstatic.com/cloudssh/images/open-btn.svg" style="max-height: 44px; margin: 32px auto; max-width: 100%;">
  </a>
</div>
## Example Usage - Memorystore Instance Desired User Created Connections
...
```
<div class = "oics-button" style="float: right; margin: 0 0 -15px">
  <a href="https://console.cloud.google.com/cloudshell/open?cloudshell_git_repo=https%3A%2F%2Fgithub.com%2Fterraform-google-modules%2Fdocs-examples.git&cloudshell_image=gcr.io%2Fcloudshell-images%2Fcloudshell%3Alatest&cloudshell_print=.%2Fmotd&cloudshell_tutorial=.%2Ftutorial.md&cloudshell_working_dir=memorystore_instance_desired_user_and_auto_created_connections&open_in_editor=main.tf" target="_blank">
    <img alt="Open in Cloud Shell" src="//gstatic.com/cloudssh/images/open-btn.svg" style="max-height: 44px; margin: 32px auto; max-width: 100%;">
  </a>
</div>
## Example Usage - Memorystore Instance Desired User And Auto Created Connections
...
* `desired_instance_endpoints` -
  (Optional)
  A list of instance endpoints
  Structure is [documented below](#nested_desired_instance_endpoints).

* `project` - (Optional) The ID of the project in which the resource belongs.
    If it is not provided, the provider project is used.


<a name="nested_desired_instance_endpoints"></a>The `desired_instance_endpoints` block supports:

* `connections` -
  (Optional)
  Structure is [documented below](#nested_desired_instance_endpoints_desired_instance_endpoints_connections).


<a name="nested_desired_instance_endpoints_desired_instance_endpoints_connections"></a>The `connections` block supports:

* `psc_connection` -
  (Optional)
  Detailed information of a PSC connection that is created by the customer
  who owns the cluster.
  Structure is [documented below](#nested_desired_instance_endpoints_desired_instance_endpoints_connections_connections_psc_connection).


<a name="nested_desired_instance_endpoints_desired_instance_endpoints_connections_connections_psc_connection"></a>The `psc_connection` block supports:
...
* `effective_instance_endpoints` -
  A list of instance endpoints
  Structure is [documented below](#nested_effective_instance_endpoints).


<a name="nested_effective_instance_endpoints"></a>The `effective_instance_endpoints` block contains:

* `connections` -
  (Optional)
  Structure is [documented below](#nested_effective_instance_endpoints_effective_instance_endpoints_connections).


<a name="nested_effective_instance_endpoints_effective_instance_endpoints_connections"></a>The `connections` block supports:

* `psc_connection` -
  (Optional)
  Detailed information of a PSC connection that is created by the customer
  who owns the cluster.
  Structure is [documented below](#nested_effective_instance_endpoints_effective_instance_endpoints_connections_connections_psc_connection).


<a name="nested_effective_instance_endpoints_effective_instance_endpoints_connections_connections_psc_connection"></a>The `psc_connection` block supports:
...
InstanceDesiredUserCreatedConnections can be imported using any of these accepted formats:
...
In Terraform v1.5.0 and later, use an [`import` block](https://developer.hashicorp.com/terraform/language/import) to import InstanceDesiredUserCreatedConnections using one of the formats above. For example:

```tf
import {
  id = "projects/{{project}}/locations/{{region}}/instances/{{name}}"
  to = google_memorystore_instance_desired_user_created_connections.default
}
```

When using the [`terraform import` command](https://developer.hashicorp.com/terraform/cli/commands/import), InstanceDesiredUserCreatedConnections can be imported using one of the formats above. For example:

```
$ terraform import google_memorystore_instance_desired_user_created_connections.default projects/{{project}}/locations/{{region}}/instances/{{name}}
$ terraform import google_memorystore_instance_desired_user_created_connections.default {{project}}/{{region}}/{{name}}
$ terraform import google_memorystore_instance_desired_user_created_connections.default {{region}}/{{name}}
$ terraform import google_memorystore_instance_desired_user_created_connections.default {{name}}
```
