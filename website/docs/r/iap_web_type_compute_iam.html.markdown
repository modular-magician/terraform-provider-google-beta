...
resource "google_iap_web_type_compute_iam_policy" "editor" {
  web_type_compute       = "projects/{{project}}/iap_web/compute"
...
* `webtypecompute` - (Required) The webtypecompute name to bind to attach 
IAM policy to.
...
* `etag` - (Computed) The etag of the webtypecompute's IAM policy.

## Import

Iap webtypecompute IAM resources can be imported using the project, webtypecompute name, role and member.
