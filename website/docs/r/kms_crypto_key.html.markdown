...
* `self_link`: The self link of the created CryptoKey. Its format is `{{keyRing}}/cryptoKeys/{{name}}`.
...
```
$ terraform import google_kms_crypto_key.default {{keyRing}}/cryptoKeys/{{name}}
$ terraform import google_kms_crypto_key.default {{keyRing}}/{{name}}
```
