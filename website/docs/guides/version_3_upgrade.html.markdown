...
Instance group managers should be using `version` blocks to reference which instance template to use for provisioning. To upgrade use a single `version` block with `instance_template` in your config and by default all traffic will be directed to that version.

### `update_strategy` has been replaced by `update_policy`

To allow much greater control over the updates happening to instance groups `update_strategy` has been replaced by `update_policy`. The previous functionality to determine if instance should be replaced or restarted can be achieved using `update_policy.minimal_action`. For more details see the [official guide.](https://cloud.google.com/compute/docs/instance-groups/rolling-out-updates-to-managed-instance-groups)
