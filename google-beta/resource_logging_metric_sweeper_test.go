...
func testSweepLoggingMetric(region string) error {
	...
	list_template := strings.Split("https://logging.googleapis.com/v2/projects/{{project}}/metrics", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://logging.googleapis.com/v2/projects/{{project}}/metrics/{{%name}}"
