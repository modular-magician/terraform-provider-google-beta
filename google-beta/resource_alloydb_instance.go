	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"instance_type": {
				...
				Required:     true,
				ForceNew:     true,
				ValidateFunc: validateEnum([]string{"INSTANCE_TYPE_UNSPECIFIED", "PRIMARY", "READ_POOL"}),
				Description:  `The type of the instance. Possible values: ["INSTANCE_TYPE_UNSPECIFIED", "PRIMARY", "READ_POOL"]`,
			},
			"annotations": {
				Type:        schema.TypeMap,
				Optional:    true,
...
func resourceAlloydbInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	url, err := replaceVars(d, config, "{{AlloydbBasePath}}projects/{{project}}/locations/{{location}}/clusters/{{cluster_id}}/instances?instanceId={{instance_id}}")
	if err != nil {
		return err
	}
	...
	// Store the ID now
	id, err := replaceVars(d, config, "projects/{{project}}/locations/{{location}}/clusters/{{cluster_id}}/instances/{{instance_id}}")
	if err != nil {
...
func resourceAlloydbInstanceRead(d *schema.ResourceData, meta interface{}) error {
	...
	url, err := replaceVars(d, config, "{{AlloydbBasePath}}projects/{{project}}/locations/{{location}}/clusters/{{cluster_id}}/instances/{{instance_id}}")
	if err != nil {
		return err
	}
...
func resourceAlloydbInstanceImport(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	...
	if err := parseImportId([]string{
		"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/clusters/(?P<cluster_id>[^/]+)/instances/(?P<instance_id>[^/]+)",
		"(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<cluster_id>[^/]+)/(?P<instance_id>[^/]+)",
		"(?P<location>[^/]+)/(?P<cluster_id>[^/]+)/(?P<instance_id>[^/]+)",
		"(?P<instance_id>[^/]+)",
	}, d, config); err != nil {
		return nil, err
	}
