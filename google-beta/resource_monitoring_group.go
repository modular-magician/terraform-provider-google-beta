...
func resourceMonitoringGroupCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return err
	} else if v := d.Get("parent_name"); !isEmptyValue(reflect.ValueOf(parentNameProp)) {
		obj["parentName"] = parentNameProp
	}
	isClusterProp, err := expandMonitoringGroupIsCluster(d.Get("is_cluster"), d, config)
	if err != nil {
		return err
	} else if v := d.Get("is_cluster"); !isEmptyValue(reflect.ValueOf(isClusterProp)) {
	...
	if err != nil {
		return err
	} else if v := d.Get("filter"); !isEmptyValue(reflect.ValueOf(filterProp)) {
