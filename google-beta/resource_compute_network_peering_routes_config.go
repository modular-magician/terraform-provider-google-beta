...
func resourceComputeNetworkPeeringRoutesConfigCreate(d *schema.ResourceData, meta interface{}) error {
	...
	// Store the ID now
	id, err := replaceVars(d, config, "projects/{{project}}/global/networks/{{network}}/networkPeerings/{{name}}")
	if err != nil {
...
func resourceComputeNetworkPeeringRoutesConfigRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("name", flattenComputeNetworkPeeringRoutesConfigName(res["name"], d)); err != nil {
		return fmt.Errorf("Error reading NetworkPeeringRoutesConfig: %s", err)
	}
...
func resourceComputeNetworkPeeringRoutesConfigImport(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	...
	if err := parseImportId([]string{
		"projects/(?P<project>[^/]+)/global/networks/(?P<network>[^/]+)/networkPeerings/(?P<name>[^/]+)",
		"(?P<project>[^/]+)/(?P<network>[^/]+)/(?P<name>[^/]+)",
		"(?P<network>[^/]+)/(?P<name>[^/]+)",
	}, d, config); err != nil {
		return nil, err
	}
...
func flattenComputeNetworkPeeringRoutesConfigName(v interface{}, d *schema.ResourceData) interface{} {
	return v
}
...
func resourceComputeNetworkPeeringRoutesConfigFindNestedObjectInList(d *schema.ResourceData, meta interface{}, items []interface{}) (index int, item map[string]interface{}, err error) {
	expectedName, err := expandComputeNetworkPeeringRoutesConfigName(d.Get("name"), d, meta.(*Config))
	if err != nil {
		return -1, nil, err
	}
	...
	for idx, itemRaw := range items {
		...
		itemName := flattenComputeNetworkPeeringRoutesConfigName(item["name"], d)
