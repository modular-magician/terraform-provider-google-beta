...
func TestAccComputeGlobalForwardingRule_labels(t *testing.T) {
	...
	fr := fmt.Sprintf("gforwardrule-test-%s", acctest.RandString(10))
	target := fmt.Sprintf("gforwardrule-test-%s", acctest.RandString(10))
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeGlobalForwardingRule_labels(fr, target),
			},
			{
				ResourceName:      "google_compute_global_forwarding_rule.forwarding_rule",
				ImportState:       true,
			...
			{
				Config: testAccComputeGlobalForwardingRule_labelsUpdated(fr, target),
			},
			{
				ResourceName:      "google_compute_global_forwarding_rule.forwarding_rule",
				ImportState:       true,
...
func testAccComputeGlobalForwardingRule_labels(fr, targetPool string) string {
	return fmt.Sprintf(`
resource "google_compute_global_forwarding_rule" "forwarding_rule" {
  name       = "%s"
  ...
  labels = {
    ...
    another-label = "my-second-label-value"
  }
}
...
func testAccComputeGlobalForwardingRule_labelsUpdated(fr, targetPool string) string {
	return fmt.Sprintf(`
resource "google_compute_global_forwarding_rule" "forwarding_rule" {
  name       = "%s"
  target     = "${google_compute_target_pool.default.self_link}"
