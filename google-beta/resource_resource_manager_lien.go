...
func resourceResourceManagerLienCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return err
	} else if v := d.Get("reason"); !isEmptyValue(reflect.ValueOf(reasonProp)) {
		obj["reason"] = reasonProp
	}
	originProp, err := expandResourceManagerLienOrigin(d.Get("origin"), d, config)
	if err != nil {
		return err
	} else if v := d.Get("origin"); !isEmptyValue(reflect.ValueOf(originProp)) {
	...
	if err != nil {
		return err
	} else if v := d.Get("restrictions"); !isEmptyValue(reflect.ValueOf(restrictionsProp)) {
