...
func testSweepBigqueryDataTransferConfig(region string) error {
	resource_name := "BigqueryDataTransferConfig"
	...
	list_template := strings.Split("https://bigquerydatatransfer.googleapis.com/v1/projects/{{project}}/locations/{{location}}/transferConfigs", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://bigquerydatatransfer.googleapis.com/v1/{{name}}"
