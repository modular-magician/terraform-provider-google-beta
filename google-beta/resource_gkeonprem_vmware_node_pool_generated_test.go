...
func TestAccGkeonpremVmwareNodePool_gkeonpremVmwareNodePoolBasicExample(t *testing.T) {
	...
	VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				ResourceName:            "google_gkeonprem_vmware_node_pool.np1",
				ImportState:             true,
...
func testAccGkeonpremVmwareNodePool_gkeonpremVmwareNodePoolBasicExample(context map[string]interface{}) string {
	return Nprintf(`
resource "google_gkeonprem_vmware_cluster" "np1" {
  vmware_cluster_id = "basic%{random_suffix}"
...
resource "google_gkeonprem_vmware_node_pool" "np1" {
  vmware_node_pool_id = "np-basic%{random_suffix}"
  name = "np-basic%{random_suffix}"
  vmware_cluster = google_gkeonprem_vmware_node_pool.np1.name
  location = "us-west1"
  config {
    replicas = 3
...
func testAccCheckGkeonpremVmwareNodePoolDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := replaceVarsForTest(config, rs, "{{GkeonpremBasePath}}projects/{{project}}/locations/{{location}}/vmwareClusters/{{vmware_cluster}}/vmwareNodePools/{{name}}")
			if err != nil {
				return err
			}
