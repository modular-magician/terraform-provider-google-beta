...
func testSweepSecurityScannerScanConfig(region string) error {
	resource_name := "SecurityScannerScanConfig"
	...
	list_template := strings.Split("https://websecurityscanner.googleapis.com/v1beta/projects/{{project}}/scanConfigs", "?")[0]
	...
	resource_list, ok := res["scanConfigs"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://websecurityscanner.googleapis.com/v1beta/{{name}}"
