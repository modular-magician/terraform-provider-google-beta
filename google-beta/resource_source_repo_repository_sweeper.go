...
func testSweepSourceRepoRepository(region string) error {
	resource_name := "SourceRepoRepository"
	...
	list_template := strings.Split("https://sourcerepo.googleapis.com/v1/projects/{{project}}/repos", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://sourcerepo.googleapis.com/v1/projects/{{project}}/repos/{{name}}"
