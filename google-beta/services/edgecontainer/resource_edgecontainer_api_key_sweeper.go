...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionEdgecontainerApiKey,
		DeleteFunction: testSweepEdgecontainerApiKey,
	}
...
func testSweepEdgecontainerApiKey(_ string) error {
	return listAndActionEdgecontainerApiKey(deleteResourceEdgecontainerApiKey)
}

func listAndActionEdgecontainerApiKey(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "EdgecontainerApiKey"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://edgecontainer.googleapis.com/v1/projects/{{project}}/locations/{{location}}/apiKeys", "?")[0]
		...
		resourceList, ok := res["apiKeys"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'apiKeys'")
		} else {
...
func deleteResourceEdgecontainerApiKey(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://edgecontainer.googleapis.com/v1/projects/{{project}}/locations/{{location}}/apiKeys/{{api_key_id}}"
