...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewSaasRuntimeTenantListResource,
	}.Register()
}

var _ list.ListResource = &SaasRuntimeTenantListResource{}

type SaasRuntimeTenantListResource struct {
	tpgresource.ListResourceMetadata
}

func NewSaasRuntimeTenantListResource() list.ListResource {
	listR := &SaasRuntimeTenantListResource{}
	listR.TypeName = "google_saas_runtime_tenant"
	listR.SDKv2Resource = ResourceSaasRuntimeTenant()
...
// SaasRuntimeTenantListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type SaasRuntimeTenantListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *SaasRuntimeTenantListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data SaasRuntimeTenantListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListSaasRuntimeTenants(
			...
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "tenant_id"); err != nil {
					return err
				}
...
func ListSaasRuntimeTenants(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceSaasRuntimeTenant().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{SaasRuntimeBasePath}}projects/{{project}}/locations/{{location}}/tenants")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceSaasRuntimeTenant(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["tenantId"]; ok && v != nil {
				if err := d.Set("tenant_id", v); err != nil {
					return fmt.Errorf("error setting tenant_id: %w", err)
				}
			}
			if err = ResourceSaasRuntimeTenantFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
