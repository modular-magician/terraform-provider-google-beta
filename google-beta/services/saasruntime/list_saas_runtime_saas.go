...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewSaasRuntimeSaasListResource,
	}.Register()
}

var _ list.ListResource = &SaasRuntimeSaasListResource{}

type SaasRuntimeSaasListResource struct {
	tpgresource.ListResourceMetadata
}

func NewSaasRuntimeSaasListResource() list.ListResource {
	listR := &SaasRuntimeSaasListResource{}
	listR.TypeName = "google_saas_runtime_saas"
	listR.SDKv2Resource = ResourceSaasRuntimeSaas()
...
// SaasRuntimeSaasListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type SaasRuntimeSaasListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *SaasRuntimeSaasListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data SaasRuntimeSaasListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListSaasRuntimeSaass(
			...
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "saas_id"); err != nil {
					return err
				}
...
func ListSaasRuntimeSaass(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceSaasRuntimeSaas().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{SaasRuntimeBasePath}}projects/{{project}}/locations/{{location}}/saas")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceSaasRuntimeSaas(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["saasId"]; ok && v != nil {
				if err := d.Set("saas_id", v); err != nil {
					return fmt.Errorf("error setting saas_id: %w", err)
				}
			}
			if err = ResourceSaasRuntimeSaasFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
