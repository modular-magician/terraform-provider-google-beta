...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewSaasRuntimeUnitKindListResource,
	}.Register()
}

var _ list.ListResource = &SaasRuntimeUnitKindListResource{}

type SaasRuntimeUnitKindListResource struct {
	tpgresource.ListResourceMetadata
}

func NewSaasRuntimeUnitKindListResource() list.ListResource {
	listR := &SaasRuntimeUnitKindListResource{}
	listR.TypeName = "google_saas_runtime_unit_kind"
	listR.SDKv2Resource = ResourceSaasRuntimeUnitKind()
...
// SaasRuntimeUnitKindListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type SaasRuntimeUnitKindListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *SaasRuntimeUnitKindListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data SaasRuntimeUnitKindListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListSaasRuntimeUnitKinds(
			...
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "unit_kind_id"); err != nil {
					return err
				}
...
func ListSaasRuntimeUnitKinds(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceSaasRuntimeUnitKind().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{SaasRuntimeBasePath}}projects/{{project}}/locations/{{location}}/unitKinds")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceSaasRuntimeUnitKind(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["unitKindId"]; ok && v != nil {
				if err := d.Set("unit_kind_id", v); err != nil {
					return fmt.Errorf("error setting unit_kind_id: %w", err)
				}
			}
			if err = ResourceSaasRuntimeUnitKindFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
