...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewSaasRuntimeUnitOperationListResource,
	}.Register()
}

var _ list.ListResource = &SaasRuntimeUnitOperationListResource{}

type SaasRuntimeUnitOperationListResource struct {
	tpgresource.ListResourceMetadata
}

func NewSaasRuntimeUnitOperationListResource() list.ListResource {
	listR := &SaasRuntimeUnitOperationListResource{}
	listR.TypeName = "google_saas_runtime_unit_operation"
	listR.SDKv2Resource = ResourceSaasRuntimeUnitOperation()
...
// SaasRuntimeUnitOperationListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type SaasRuntimeUnitOperationListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *SaasRuntimeUnitOperationListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data SaasRuntimeUnitOperationListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListSaasRuntimeUnitOperations(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "unit_operation_id"); err != nil {
					return err
				}
...
func ListSaasRuntimeUnitOperations(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceSaasRuntimeUnitOperation().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{SaasRuntimeBasePath}}projects/{{project}}/locations/{{location}}/unitOperations")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceSaasRuntimeUnitOperation(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "unitOperations",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["unitOperationId"]; ok && v != nil {
				if err := d.Set("unit_operation_id", v); err != nil {
					return fmt.Errorf("error setting unit_operation_id: %w", err)
				}
			}
			if err = ResourceSaasRuntimeUnitOperationFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
