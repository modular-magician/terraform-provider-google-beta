...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewSaasRuntimeReleaseListResource,
	}.Register()
}

var _ list.ListResource = &SaasRuntimeReleaseListResource{}

type SaasRuntimeReleaseListResource struct {
	tpgresource.ListResourceMetadata
}

func NewSaasRuntimeReleaseListResource() list.ListResource {
	listR := &SaasRuntimeReleaseListResource{}
	listR.TypeName = "google_saas_runtime_release"
	listR.SDKv2Resource = ResourceSaasRuntimeRelease()
...
// SaasRuntimeReleaseListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type SaasRuntimeReleaseListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *SaasRuntimeReleaseListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data SaasRuntimeReleaseListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListSaasRuntimeReleases(
			...
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "release_id"); err != nil {
					return err
				}
...
func ListSaasRuntimeReleases(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceSaasRuntimeRelease().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{SaasRuntimeBasePath}}projects/{{project}}/locations/{{location}}/releases")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceSaasRuntimeRelease(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["releaseId"]; ok && v != nil {
				if err := d.Set("release_id", v); err != nil {
					return fmt.Errorf("error setting release_id: %w", err)
				}
			}
			if err = ResourceSaasRuntimeReleaseFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
