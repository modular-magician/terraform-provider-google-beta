...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewSaasRuntimeUnitListResource,
	}.Register()
}

var _ list.ListResource = &SaasRuntimeUnitListResource{}

type SaasRuntimeUnitListResource struct {
	tpgresource.ListResourceMetadata
}

func NewSaasRuntimeUnitListResource() list.ListResource {
	listR := &SaasRuntimeUnitListResource{}
	listR.TypeName = "google_saas_runtime_unit"
	listR.SDKv2Resource = ResourceSaasRuntimeUnit()
...
// SaasRuntimeUnitListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type SaasRuntimeUnitListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *SaasRuntimeUnitListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data SaasRuntimeUnitListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListSaasRuntimeUnits(
			...
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "unit_id"); err != nil {
					return err
				}
...
func ListSaasRuntimeUnits(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceSaasRuntimeUnit().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{SaasRuntimeBasePath}}projects/{{project}}/locations/{{location}}/units")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceSaasRuntimeUnit(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["unitId"]; ok && v != nil {
				if err := d.Set("unit_id", v); err != nil {
					return fmt.Errorf("error setting unit_id: %w", err)
				}
			}
			if err = ResourceSaasRuntimeUnitFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
