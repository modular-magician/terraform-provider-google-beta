...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkServicesServiceLbPoliciesListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesServiceLbPoliciesListResource{}

type NetworkServicesServiceLbPoliciesListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesServiceLbPoliciesListResource() list.ListResource {
	listR := &NetworkServicesServiceLbPoliciesListResource{}
	listR.TypeName = "google_network_services_service_lb_policies"
	listR.SDKv2Resource = ResourceNetworkServicesServiceLbPolicies()
...
// NetworkServicesServiceLbPoliciesListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesServiceLbPoliciesListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkServicesServiceLbPoliciesListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesServiceLbPoliciesListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkServicesServiceLbPoliciess(
			listR.Client,
			location,
			project,
...
func ListNetworkServicesServiceLbPoliciess(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesServiceLbPolicies().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/{{location}}/serviceLbPolicies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesServiceLbPolicies(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "serviceLbPolicies",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkServicesServiceLbPoliciesFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
