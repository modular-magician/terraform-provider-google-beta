...
func TestAccNetworkServicesServiceLbPoliciesListQuery_generated(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_network_services_service_lb_policies.default",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_network_services_service_lb_policies.default",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccNetworkServicesServiceLbPolicies_networkServicesServiceLbPoliciesBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_network_services_service_lb_policies.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_network_services_service_lb_policies.list_query",
...
func testAccNetworkServicesServiceLbPolicies_networkServicesServiceLbPoliciesBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_network_services_service_lb_policies" "list_query" {
