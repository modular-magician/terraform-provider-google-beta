...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_cloud_trace_trace_sink",
		ListAndAction:  listAndActionCloudTraceTraceSink,
		DeleteFunction: testSweepCloudTraceTraceSink,
	}
...
func testSweepCloudTraceTraceSink(_ string) error {
	return listAndActionCloudTraceTraceSink(deleteResourceCloudTraceTraceSink)
}

func listAndActionCloudTraceTraceSink(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "CloudTraceTraceSink"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://cloudtrace.googleapis.com/v2beta1/projects/{{project}}/traceSinks", "?")[0]
		...
		resourceList, ok := res["traceSinks"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'traceSinks'")
		} else {
...
func deleteResourceCloudTraceTraceSink(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://cloudtrace.googleapis.com/v2beta1/projects/{{project}}//traceSinks/{{name}}"
