...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionCloudTraceSink,
		DeleteFunction: testSweepCloudTraceSink,
	}
...
func testSweepCloudTraceSink(_ string) error {
	return listAndActionCloudTraceSink(deleteResourceCloudTraceSink)
}

func listAndActionCloudTraceSink(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "CloudTraceSink"
	...
	for _, mockConfig := range configs {
		...
		resourceList, ok := res["sinks"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'sinks'")
		} else {
...
func deleteResourceCloudTraceSink(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://cloudtrace.googleapis.com/v2beta1/projects/{{project}}/traceSinks/{{sink_id}}"
