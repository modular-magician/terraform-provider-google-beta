...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetappkmsconfig,
		DeleteFunction: testSweepNetappkmsconfig,
	}
	// Add dependencies
...
func testSweepNetappkmsconfig(_ string) error {
	return listAndActionNetappkmsconfig(deleteResourceNetappkmsconfig)
}

func listAndActionNetappkmsconfig(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'kmsConfigs'")
		} else {
...
func deleteResourceNetappkmsconfig(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
