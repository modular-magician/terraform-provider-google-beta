...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionAgentRegistryMcpServer,
		DeleteFunction: testSweepAgentRegistryMcpServer,
	}
...
func testSweepAgentRegistryMcpServer(_ string) error {
	return listAndActionAgentRegistryMcpServer(deleteResourceAgentRegistryMcpServer)
}

func listAndActionAgentRegistryMcpServer(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "AgentRegistryMcpServer"
	...
	for _, mockConfig := range configs {
		...
		resourceList, ok := res["mcpServers"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'mcpServers'")
		} else {
...
func deleteResourceAgentRegistryMcpServer(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://agentregistry.googleapis.com/v1alpha/projects/{{project}}/locations/{{location}}/services/{{mcp_server_id}}"
