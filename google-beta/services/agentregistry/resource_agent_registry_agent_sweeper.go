...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionAgentRegistryAgent,
		DeleteFunction: testSweepAgentRegistryAgent,
	}
...
func testSweepAgentRegistryAgent(_ string) error {
	return listAndActionAgentRegistryAgent(deleteResourceAgentRegistryAgent)
}

func listAndActionAgentRegistryAgent(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "AgentRegistryAgent"
	...
	for _, mockConfig := range configs {
		...
		resourceList, ok := res["agents"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'agents'")
		} else {
...
func deleteResourceAgentRegistryAgent(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://agentregistry.googleapis.com/v1alpha/projects/{{project}}/locations/{{location}}/services/{{agent_id}}"
