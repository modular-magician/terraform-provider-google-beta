...
func TestAccSecurityScannerScanConfig_scanConfigRiskLevelExample(t *testing.T) {
	t.Parallel()

	randomSuffix := acctest.RandString(t, 10)

	context := map[string]interface{}{
		"address_name":     "tf-test-scan-risk-level-ip" + randomSuffix,
		"scan_config_name": "tf-test-scan-config-risk-level" + randomSuffix,
		"random_suffix":    randomSuffix,
	}
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSecurityScannerScanConfig_scanConfigRiskLevelExample(context),
			},
			{
				ResourceName:      "google_security_scanner_scan_config.scan-config",
				ImportState:       true,
...
func testAccSecurityScannerScanConfig_scanConfigRiskLevelExample(context map[string]interface{}) string {
	return acctest.Nprintf(`
resource "google_compute_address" "scanner_static_ip" {
  provider = google-beta
  name     = "%{address_name}"
