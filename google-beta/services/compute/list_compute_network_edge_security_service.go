...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeNetworkEdgeSecurityServiceListResource,
	}.Register()
}

var _ list.ListResource = &ComputeNetworkEdgeSecurityServiceListResource{}

type ComputeNetworkEdgeSecurityServiceListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeNetworkEdgeSecurityServiceListResource() list.ListResource {
	listR := &ComputeNetworkEdgeSecurityServiceListResource{}
	listR.TypeName = "google_compute_network_edge_security_service"
	listR.SDKv2Resource = ResourceComputeNetworkEdgeSecurityService()
...
// ComputeNetworkEdgeSecurityServiceListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeNetworkEdgeSecurityServiceListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeNetworkEdgeSecurityServiceListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeNetworkEdgeSecurityServiceListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeNetworkEdgeSecurityServices(
			listR.Client,
			region,
			project,
...
func ListComputeNetworkEdgeSecurityServices(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeNetworkEdgeSecurityService().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/networkEdgeSecurityServices")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeNetworkEdgeSecurityService(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "networkEdgeSecurityServices",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeNetworkEdgeSecurityServiceFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
