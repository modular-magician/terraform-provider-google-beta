...
func resourceComputeFirewallPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	url, err := tpgresource.ReplaceVars(d, config, "{{ComputeBasePath}}locations/global/firewallPolicies/{{name}}?parentId={{parent}}")
	if err != nil {
		return err
	}
	...
	if err := d.Set("firewall_policy_id", flattenComputeFirewallPolicyFirewallPolicyId(res["firewallPolicyId"], d, config)); err != nil {
		return fmt.Errorf("Error reading FirewallPolicy: %s", err)
	}
	if err := d.Set("name", flattenComputeFirewallPolicyName(res["name"], d, config)); err != nil {
		return fmt.Errorf("Error reading FirewallPolicy: %s", err)
