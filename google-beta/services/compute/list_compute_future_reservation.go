...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeFutureReservationListResource,
	}.Register()
}

var _ list.ListResource = &ComputeFutureReservationListResource{}

type ComputeFutureReservationListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeFutureReservationListResource() list.ListResource {
	listR := &ComputeFutureReservationListResource{}
	listR.TypeName = "google_compute_future_reservation"
	listR.SDKv2Resource = ResourceComputeFutureReservation()
...
// ComputeFutureReservationListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeFutureReservationListModel struct {
	Zone    types.String `tfsdk:"zone"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeFutureReservationListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeFutureReservationListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeFutureReservations(
			listR.Client,
			zone,
			project,
...
func ListComputeFutureReservations(config *transport_tpg.Config,
	zone string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeFutureReservation().Data(&terraform.InstanceState{})
	if zone != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/zones/{{zone}}/futureReservations")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeFutureReservation(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeFutureReservationFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
