...
func testAccCheckComputeInstanceHasAliasIpv6Range(instance *map[string]interface{}, subnetworkRangeName, iPCidrRange string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		nics, _ := (*instance)["networkInterfaces"].([]interface{})
		for _, rawNi := range nics {
			ni, ok := rawNi.(map[string]interface{})
		...
		return fmt.Errorf("Alias ipv6 range with name %s and cidr %s not present", subnetworkRangeName, iPCidrRange)
	}
}
