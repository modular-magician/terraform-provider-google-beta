...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRegionNetworkPolicyListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRegionNetworkPolicyListResource{}

type ComputeRegionNetworkPolicyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRegionNetworkPolicyListResource() list.ListResource {
	listR := &ComputeRegionNetworkPolicyListResource{}
	listR.TypeName = "google_compute_region_network_policy"
	listR.SDKv2Resource = ResourceComputeRegionNetworkPolicy()
...
// ComputeRegionNetworkPolicyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRegionNetworkPolicyListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRegionNetworkPolicyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRegionNetworkPolicyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeRegionNetworkPolicys(
			listR.Client,
			region,
			project,
...
func ListComputeRegionNetworkPolicys(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRegionNetworkPolicy().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/networkPolicies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRegionNetworkPolicy(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "networkPolicies",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeRegionNetworkPolicyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
