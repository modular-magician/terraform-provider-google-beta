...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_saas_saas",
		ListAndAction:  listAndActionSaasSaas,
		DeleteFunction: testSweepSaasSaas,
	}
...
func testSweepSaasSaas(_ string) error {
	return listAndActionSaasSaas(deleteResourceSaasSaas)
}

func listAndActionSaasSaas(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "SaasSaas"
	...
	for _, mockConfig := range configs {
		...
		resourceList, ok := res["saass"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'saass'")
		} else {
...
func deleteResourceSaasSaas(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
