...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkSecurityAuthorizationPolicyListResource,
	}.Register()
}

var _ list.ListResource = &NetworkSecurityAuthorizationPolicyListResource{}

type NetworkSecurityAuthorizationPolicyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkSecurityAuthorizationPolicyListResource() list.ListResource {
	listR := &NetworkSecurityAuthorizationPolicyListResource{}
	listR.TypeName = "google_network_security_authorization_policy"
	listR.SDKv2Resource = ResourceNetworkSecurityAuthorizationPolicy()
...
// NetworkSecurityAuthorizationPolicyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkSecurityAuthorizationPolicyListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *NetworkSecurityAuthorizationPolicyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkSecurityAuthorizationPolicyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkSecurityAuthorizationPolicys(
			listR.Client,
			project,
...
func ListNetworkSecurityAuthorizationPolicys(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkSecurityAuthorizationPolicy().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkSecurityBasePath}}projects/{{project}}/locations/global/authorizationPolicies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkSecurityAuthorizationPolicy(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "authorizationPolicies",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkSecurityAuthorizationPolicyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
			id, err := tpgresource.ReplaceVars(d, config, "projects/{{project}}/locations/global/authorizationPolicies/{{name}}")
			if err != nil {
