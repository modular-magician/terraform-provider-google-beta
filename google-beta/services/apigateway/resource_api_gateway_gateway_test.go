...
func TestAccAPIGatewayGateway_tags(t *testing.T) {
	t.Parallel()
	tagKey := acctest.BootstrapSharedTestOrganizationTagKey(t, "APIGateway-Gateway-tagkey", map[string]interface{}{})
	context := map[string]interface{}{
		"random_suffix": acctest.RandString(t, 10),
		"org":           envvar.GetTestOrgFromEnv(t),
		"tagKey":        tagKey,
		"tagValue":      acctest.BootstrapSharedTestOrganizationTagValue(t, "APIGateway-Gateway-tagvalue", tagKey),
	}
	acctest.VcrTest(t, resource.TestCase{
		PreCheck:                 func() { acctest.AccTestPreCheck(t) },
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAPIGatewayGatewayTags(context),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("google_api_gateway_gateway.default", "tags.%"),
					testAccCheckAPIGatewayGatewayHasTagBindings(t),
				),
			},
			{
				ResourceName:            "google_api_gateway_gateway.default",
				ImportState:             true,
				ImportStateVerify:       true,
				ImportStateVerifyIgnore: []string{"provider", "api_config", "gateway_id", "tags"},
			},
		},
	})
}

func testAccCheckAPIGatewayGatewayHasTagBindings(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			respDescribe, err := transport_tpg.SendRequest(transport_tpg.SendRequestOptions{
				...
				UserAgent: "google_api_gateway_gateway", // Updated UserAgent
			})
			...
			// 3. Get the tag bindings from the APIGateway Gateway.
			...
			parentURL := fmt.Sprintf("//apigateway.googleapis.com/projects/%s/locations/%s/clusters/%s", project, location, cluster_id)
...
func testAccAPIGatewayGatewayTags(context map[string]interface{}) string {
	return acctest.Nprintf(`
resource "google_api_gateway_api" "api_gw" {
  provider = google-beta
  api_id = "tf-test-my-api%{random_suffix}"
