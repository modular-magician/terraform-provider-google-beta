...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVertexAIPipelineJob,
		DeleteFunction: testSweepVertexAIPipelineJob,
	}
...
func testSweepVertexAIPipelineJob(_ string) error {
	return listAndActionVertexAIPipelineJob(deleteResourceVertexAIPipelineJob)
}

func listAndActionVertexAIPipelineJob(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VertexAIPipelineJob"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/pipelineJobs", "?")[0]
		...
		resourceList, ok := res["pipelineJobs"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'pipelineJobs'")
		} else {
...
func deleteResourceVertexAIPipelineJob(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/pipelineJobs/{{name}}"
