...
func testSweepComputeRegionSslCertificate(region string) error {
	resource_name := "ComputeRegionSslCertificate"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/sslCertificates", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/sslCertificates/{{name}}"
