...
func testSweepHealthcareDataset(region string) error {
	resource_name := "HealthcareDataset"
	...
	list_template := strings.Split("https://healthcare.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/datasets?datasetId={{name}}", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://healthcare.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/datasets/{{name}}"
