...
func TestAccRedisInstance_redisInstanceScheduledRdbSnapshotExample(t *testing.T) {
	t.Parallel()

	context := map[string]interface{}{
		"network_name":  BootstrapSharedTestNetwork(t, "redis-scheduled-rdb-snapshot"),
		"random_suffix": randString(t, 10),
	}
	...
	vcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccRedisInstance_redisInstanceScheduledRdbSnapshotExample(context),
			},
			{
				ResourceName:            "google_redis_instance.cache",
				ImportState:             true,
...
func testAccRedisInstance_redisInstanceScheduledRdbSnapshotExample(context map[string]interface{}) string {
	return Nprintf(`
resource "google_redis_instance" "cache" {
  name           = "tf-test-mrr-scheduled-rdb-snapshot%{random_suffix}"
  tier           = "STANDARD_HA"
  memory_size_gb = 1
