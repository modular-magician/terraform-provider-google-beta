...
func testSweepCloudSchedulerJob(region string) error {
	resource_name := "CloudSchedulerJob"
	...
	list_template := strings.Split("https://cloudscheduler.googleapis.com/v1/projects/{{project}}/locations/{{region}}/jobs", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://cloudscheduler.googleapis.com/v1/projects/{{project}}/locations/{{region}}/jobs/{{name}}"
