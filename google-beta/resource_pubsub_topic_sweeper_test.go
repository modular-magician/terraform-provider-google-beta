...
func testSweepPubsubTopic(region string) error {
	...
	list_template := strings.Split("https://pubsub.googleapis.com/v1/projects/{{project}}/topics", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://pubsub.googleapis.com/v1/projects/{{project}}/topics/{{name}}"
