...
// TODO(emilyye): Fix this test once we move network endpoint group/network
// endpoint to use the normal GA providers and properly check create/deletion.
// With non-GA-only resources, f we destroy an endpoint during a TestStep,
// Terraform will add an implicit provider dependency on the GA provider
// instead of the non-GA provider, as the resource no longer exists in config
// and thus no longer has the explicit provider declaration.
func TestAccComputeNetworkEndpoint_networkEndpointsBasic(t *testing.T) {
	...
	var initialEndpt *networkEndptTestId
	var modifiedEndpt *networkEndptTestId
	var addedEndpt *networkEndptTestId
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					// Store information about initially created endpoint in initialEndpt
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					// Make sure initial created endpoint (initialEndpt) was
					// destroyed (in force-new operation). We do this here
					// instead of CheckDestroy so the parent network endpoint
					// group still exists
					testAccCheckComputeNetworkEndpointDestroyed("google_compute_network_endpoint.default-endpoint", initialEndpt),
					// Make sure modified endpoint and newly added endpoint exists.
					testAccCheckComputeNetworkEndpointExists("google_compute_network_endpoint.default-endpoint", modifiedEndpt),
					testAccCheckComputeNetworkEndpointExists("google_compute_network_endpoint.new-endpoint", addedEndpt),
				),
			},
		},
	})
...
resource "google_compute_network_endpoint" "new-endpoint" {
 provider               = "google-beta"
