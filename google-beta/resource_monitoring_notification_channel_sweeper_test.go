...
func testSweepMonitoringNotificationChannel(region string) error {
	resource_name := "MonitoringNotificationChannel"
	...
	list_template := strings.Split("https://monitoring.googleapis.com/v3/projects/{{project}}/notificationChannels", "?")[0]
	...
	resource_list, ok := res["notificationChannels"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://monitoring.googleapis.com/v3/{{name}}"
