...
func testSweepCloudBuildTrigger(region string) error {
	resource_name := "CloudBuildTrigger"
	...
	list_template := strings.Split("https://cloudbuild.googleapis.com/v1/projects/{{project}}/triggers", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://cloudbuild.googleapis.com/v1/projects/{{project}}/triggers/{{trigger_id}}"
