...
func resourceCloudBuildTriggerCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return err
	} else if v := d.Get("included_files"); !isEmptyValue(reflect.ValueOf(includedFilesProp)) {
	...
	if err != nil {
		return err
	} else if v := d.Get("trigger_template"); !isEmptyValue(reflect.ValueOf(triggerTemplateProp)) {
		obj["triggerTemplate"] = triggerTemplateProp
	}
	buildProp, err := expandCloudBuildTriggerBuild(d.Get("build"), d, config)
	if err != nil {
		return err
	} else if v := d.Get("build"); !isEmptyValue(reflect.ValueOf(buildProp)) {
		obj["build"] = buildProp
	}
