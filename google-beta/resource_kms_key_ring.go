...
func resourceKmsKeyRingRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("location", flattenKmsKeyRingLocation(res["location"], d)); err != nil {
		return fmt.Errorf("Error reading KeyRing: %s", err)
	}
...
func resourceKmsKeyRingImport(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	config := meta.(*Config)
	if err := parseImportId([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/keyRings/(?P<name>[^/]+)", "projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/keyRings?keyRingId=(?P<name>[^/]+)/(?P<name>[^/]+)", "(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<name>[^/]+)/(?P<name>[^/]+)", "(?P<location>[^/]+)/(?P<name>[^/]+)/(?P<name>[^/]+)"}, d, config); err != nil {
		return nil, err
	}
...
func flattenKmsKeyRingLocation(v interface{}, d *schema.ResourceData) interface{} {
	return v
}
...
func resourceKmsKeyRingDecoder(d *schema.ResourceData, meta interface{}, res map[string]interface{}) (map[string]interface{}, error) {
	config := meta.(*Config)
	res["self_link"] = d.Get("name").(string)
	d.SetId(res["name"].(string))
	if err := parseImportId([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/keyRings/(?P<name>[^/]+)"}, d, config); err != nil {
		return nil, err
	}
	res["project"] = d.Get("project").(string)
	res["location"] = d.Get("location").(string)
