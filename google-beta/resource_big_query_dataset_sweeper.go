...
func testSweepBigQueryDataset(region string) error {
	...
	list_template := strings.Split("https://www.googleapis.com/bigquery/v2/projects/{{project}}/datasets", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/bigquery/v2/projects/{{project}}/datasets/{{dataset_id}}?deleteContents={{delete_contents_on_destroy}}"
