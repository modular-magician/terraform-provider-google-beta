...
func testAccCheckHealthcareDatasetDestroy(s *terraform.State) error {
	for name, rs := range s.RootModule().Resources {
		...
		url, err := replaceVarsForTest(rs, "{{HealthcareBasePath}}projects/{{project}}/locations/{{location}}/datasets/{{name}}")
		if err != nil {
			return err
		}
...
func testAccCheckGoogleHealthcareDatasetUpdate(timeZone string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		for _, rs := range s.RootModule().Resources {
			...
			gcpResourceUri, err := replaceVarsForTest(rs, "projects/{{project}}/locations/{{location}}/datasets/{{name}}")
			if err != nil {
				return err
			}
