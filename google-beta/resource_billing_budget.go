	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			"billing_account": {
				...
				Required:    true,
				ForceNew:    true,
				Description: `ID the billing account to set budget on.`,
			},
			"amount": {
...
1.0-based percentage, so 0.5 = 50%. Must be >= 0.`,
						...
						"spend_basis": {
							Type:         schema.TypeString,
							Optional:     true,
							ValidateFunc: validation.StringInSlice([]string{"BASIS_UNSPECIFIED", "CURRENT_SPEND", "FORECASTED_SPEND", ""}, false),
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"credit_types_treatment": {
							Type:         schema.TypeString,
							Optional:     true,
							ValidateFunc: validation.StringInSlice([]string{"CREDIT_TYPES_TREATMENT_UNSPECIFIED", "INCLUDE_ALL_CREDITS", "EXCLUDE_ALL_CREDITS", ""}, false),
...
https://cloud.google.com/billing/v1/how-tos/catalog-api.`,
			...
			"etag": {
				Type:     schema.TypeString,
				Computed: true,
				Description: `Etag to validate that the object is unchanged for a
...
func resourceBillingBudgetRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("etag", flattenBillingBudgetEtag(res["etag"], d)); err != nil {
		return fmt.Errorf("Error reading Budget: %s", err)
	}
...
func resourceBillingBudgetUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("budget") {
		updateMask = append(updateMask, "budget")
	}
...
func flattenBillingBudgetEtag(v interface{}, d *schema.ResourceData) interface{} {
	return v
}
...
func expandBillingBudgetBudgetThresholdRules(v interface{}, d TerraformResourceData, config *Config) (interface{}, error) {
	...
	for _, raw := range l {
		...
		if err != nil {
			return nil, err
		} else if val := reflect.ValueOf(transformedThresholdPercent); val.IsValid() && !isEmptyValue(val) {
