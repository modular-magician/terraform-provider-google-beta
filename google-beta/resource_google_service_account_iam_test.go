...
func TestAccServiceAccountIamBinding_update(t *testing.T) {
	t.Parallel()

	account := acctest.RandomWithPrefix("tf-test")
	conditionExpr := `request.time < timestamp(\"2020-01-01T00:00:00Z\")`
	conditionExpr2 := `request.time < timestamp(\"2021-01-01T00:00:00Z\")`
	conditionTitle := "expires_after_2019_12_31"
	conditionTitle2 := "expires_after_2020_12_31"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Update condition member list
			{
				Config: testAccServiceAccountIamBinding_withCondition(account, "user:paddy@hashicorp.com", conditionTitle, conditionExpr),
				Check:  testAccCheckGoogleServiceAccountIam(account, 1),
			},
			{
				ResourceName:      "google_service_account_iam_binding.foo",
			...
			{
				Config: testAccServiceAccountIamBinding_withCondition(account, "user:paddy@hashicorp.com", conditionTitle, conditionExpr2),
				Check:  testAccCheckGoogleServiceAccountIam(account, 1),
			},
			{
				ResourceName:      "google_service_account_iam_binding.foo",
			...
			{
				Config: testAccServiceAccountIamBinding_withCondition(account, "user:paddy@hashicorp.com", conditionTitle2, conditionExpr2),
				Check:  testAccCheckGoogleServiceAccountIam(account, 1),
			},
			{
				ResourceName:      "google_service_account_iam_binding.foo",
...
func TestAccServiceAccountIamMember_withCondition(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccServiceAccountIamMember_withCondition(account),
				Check:  testAccCheckGoogleServiceAccountIam(account, 1),
			},
			{
				ResourceName:      "google_service_account_iam_member.foo",
...
func testAccServiceAccountIamMember_withCondition(account string) string {
	return fmt.Sprintf(`
resource "google_service_account" "test_account" {
  account_id   = "%s"
  display_name = "Service Account Iam Testing Account"
