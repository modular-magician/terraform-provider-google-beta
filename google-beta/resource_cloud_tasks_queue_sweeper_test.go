...
func testSweepCloudTasksQueue(region string) error {
	...
	list_template := strings.Split("https://cloudtasks.googleapis.com/v2/projects/{{project}}/locations/{{location}}/queues", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://cloudtasks.googleapis.com/v2/projects/{{project}}/locations/{{location}}/queues/{{name}}"
