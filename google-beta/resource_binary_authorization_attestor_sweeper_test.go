...
func testSweepBinaryAuthorizationAttestor(region string) error {
	...
	list_template := strings.Split("https://binaryauthorization.googleapis.com/v1beta1/projects/{{project}}/attestors?attestorId={{name}}", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://binaryauthorization.googleapis.com/v1beta1/projects/{{project}}/attestors/{{name}}"
