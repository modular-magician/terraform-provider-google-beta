...
func testSweepVPCAccessConnector(region string) error {
	resource_name := "VPCAccessConnector"
	...
	list_template := strings.Split("https://vpcaccess.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/connectors", "?")[0]
	...
	resource_list, ok := res["connectors"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://vpcaccess.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/connectors/{{name}}"
