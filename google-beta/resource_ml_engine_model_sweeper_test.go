...
func testSweepMLEngineModel(region string) error {
	...
	list_template := strings.Split("https://ml.googleapis.com/v1/projects/{{project}}/models", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://ml.googleapis.com/v1/projects/{{project}}/models/{{name}}"
