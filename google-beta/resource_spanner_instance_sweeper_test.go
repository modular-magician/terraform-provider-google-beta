...
func testSweepSpannerInstance(region string) error {
	...
	list_template := strings.Split("https://spanner.googleapis.com/v1/projects/{{project}}/instances", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://spanner.googleapis.com/v1/projects/{{project}}/instances/{{name}}"
