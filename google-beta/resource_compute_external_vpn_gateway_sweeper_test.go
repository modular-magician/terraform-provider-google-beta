...
func testSweepComputeExternalVpnGateway(region string) error {
	resource_name := "ComputeExternalVpnGateway"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/beta/projects/{{project}}/global/externalVpnGateways", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/beta/projects/{{project}}/global/externalVpnGateways/{{name}}"
