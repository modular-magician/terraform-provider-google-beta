...
func testSweepHealthcareHl7V2Store(region string) error {
	resource_name := "HealthcareHl7V2Store"
	...
	list_template := strings.Split("https://healthcare.googleapis.com/v1beta1/{{dataset}}/hl7V2Stores?hl7V2StoreId={{name}}", "?")[0]
	...
	resource_list, ok := res["hl7V2Stores"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://healthcare.googleapis.com/v1beta1/{{dataset}}/hl7V2Stores/{{name}}"
