...
func testSweepDeploymentManagerDeployment(region string) error {
	resource_name := "DeploymentManagerDeployment"
	...
	list_template := strings.Split("https://www.googleapis.com/deploymentmanager/v2/projects/{{project}}/global/deployments", "?")[0]
	...
	resource_list, ok := res["deployments"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/deploymentmanager/v2/projects/{{project}}/global/deployments/{{name}}?deletePolicy={{delete_policy}}"
