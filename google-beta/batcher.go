...
func (b *RequestBatcher) SendRequestWithTimeout(batchKey string, request *BatchRequest, timeout time.Duration) (interface{}, error) {
	...
	case resp := <-respCh:
		if resp.err != nil {
			...
			errMsg := fmt.Sprintf(
				"Batch %q for request %q returned error: {{err}}. To disable batching see https://www.terraform.io/docs/providers/google/guides/provider_reference.html#enable_batching",
