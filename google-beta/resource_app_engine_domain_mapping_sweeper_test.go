...
func testSweepAppEngineDomainMapping(region string) error {
	resource_name := "AppEngineDomainMapping"
	...
	list_template := strings.Split("https://appengine.googleapis.com/v1/apps/{{project}}/domainMappings", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://appengine.googleapis.com/v1/apps/{{project}}/domainMappings/{{domain_name}}"
