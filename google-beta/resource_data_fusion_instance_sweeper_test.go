...
func testSweepDataFusionInstance(region string) error {
	resource_name := "DataFusionInstance"
	...
	list_template := strings.Split("https://datafusion.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/instances", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://datafusion.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/instances/{{name}}"
