...
func testSweepPubsubSubscription(region string) error {
	resource_name := "PubsubSubscription"
	...
	list_template := strings.Split("https://pubsub.googleapis.com/v1/projects/{{project}}/subscriptions", "?")[0]
	...
	resource_list, ok := res["subscriptions"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://pubsub.googleapis.com/v1/projects/{{project}}/subscriptions/{{name}}"
