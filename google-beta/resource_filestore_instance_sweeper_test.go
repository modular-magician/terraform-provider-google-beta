...
func testSweepFilestoreInstance(region string) error {
	resource_name := "FilestoreInstance"
	...
	list_template := strings.Split("https://file.googleapis.com/v1/projects/{{project}}/locations/{{zone}}/instances", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://file.googleapis.com/v1/projects/{{project}}/locations/{{zone}}/instances/{{name}}"
