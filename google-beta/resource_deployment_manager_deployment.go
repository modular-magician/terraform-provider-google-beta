...
actually change the deployment, just how it is updated.`,
			...
			"labels": {
				Type:        schema.TypeSet,
				Optional:    true,
				Description: `Key-value pairs to apply to this address.`,
...
configuration.`,
			...
			"fingerprint": {
				Type:     schema.TypeString,
				Computed: true,
				Description: `Output only. Fingerprint generated by Deployment Manager for use in
...
func resourceDeploymentManagerDeploymentCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		log.Printf("[WARN] Checking if Deployment %q was created with failed state", id)
		var cleanErr error
		if cleanErr = resourceDeploymentManagerDeploymentRead(d, meta); cleanErr == nil {
			if d.Id() != "" {
				log.Printf("[WARN] Deployment %q still exists, attempting to delete...", id)
				if cleanErr = resourceDeploymentManagerDeploymentDelete(d, meta); cleanErr == nil {
					log.Printf("[WARN] failed Deployment %q was successfully deleted", id)
				}
			}
		}
		if cleanErr != nil {
			log.Printf("[WARN] Could not confirm cleanup of Deployment %s if created in error state: %v", id, cleanErr)
		}
...
func resourceDeploymentManagerDeploymentRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("fingerprint", flattenDeploymentManagerDeploymentFingerprint(res["fingerprint"], d)); err != nil {
		return fmt.Errorf("Error reading Deployment: %s", err)
	}
...
func flattenDeploymentManagerDeploymentFingerprint(v interface{}, d *schema.ResourceData) interface{} {
	return v
}
