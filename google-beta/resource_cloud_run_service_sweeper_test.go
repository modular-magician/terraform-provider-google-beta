...
func testSweepCloudRunService(region string) error {
	...
	list_template := strings.Split("https://{{location}}-run.googleapis.com/apis/serving.knative.dev/v1/namespaces/{{project}}/services", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://{{location}}-run.googleapis.com/apis/serving.knative.dev/v1/namespaces/{{project}}/services/{{name}}"
