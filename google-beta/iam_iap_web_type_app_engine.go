...
func IapWebTypeAppEngineIamUpdaterProducer(d *schema.ResourceData, config *Config) (ResourceIamUpdater, error) {
	...
	m, err := getImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/iap_web/appengine-(?P<appEngineId>[^/]+)", "(?P<project>[^/]+)/(?P<appEngineId>[^/]+)", "(?P<appEngineId>[^/]+)"}, d, config, d.Get("app_engine_id").(string))
	...
	if _, found := values["appEngineId"]; !found {
		if v, ok := d.GetOkExists("app_engine_id"); ok {
			values["appEngineId"] = v.(string)
		}
	}
	...
	d.Set("app_engine_id", u.GetResourceId())
...
func IapWebTypeAppEngineIdParseFunc(d *schema.ResourceData, config *Config) error {
	...
	m, err := getImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/iap_web/appengine-(?P<appEngineId>[^/]+)", "(?P<project>[^/]+)/(?P<appEngineId>[^/]+)", "(?P<appEngineId>[^/]+)"}, d, config, d.Id())
	if err != nil {
		return err
	}
...
func (u *IapWebTypeAppEngineIamUpdater) qualifyWebTypeAppEngineUrl(methodIdentifier string) string {
	return fmt.Sprintf("https://iap.googleapis.com/v1/%s:%s", fmt.Sprintf("projects/%s/iap_web/appengine-%s", u.project, u.app_engine_id), methodIdentifier)
}
