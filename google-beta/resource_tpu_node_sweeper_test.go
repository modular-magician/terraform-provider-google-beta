...
func testSweepTPUNode(region string) error {
	...
	list_template := strings.Split("https://tpu.googleapis.com/v1/projects/{{project}}/locations/{{zone}}/nodes", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://tpu.googleapis.com/v1/projects/{{project}}/locations/{{zone}}/nodes/{{name}}"
