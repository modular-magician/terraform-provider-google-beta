...
func testSweepHealthcareFhirStore(region string) error {
	resource_name := "HealthcareFhirStore"
	...
	list_template := strings.Split("https://healthcare.googleapis.com/v1beta1/{{dataset}}/fhirStores?fhirStoreId={{name}}", "?")[0]
	...
	resource_list, ok := res["fhirStores"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://healthcare.googleapis.com/v1beta1/{{dataset}}/fhirStores/{{name}}"
