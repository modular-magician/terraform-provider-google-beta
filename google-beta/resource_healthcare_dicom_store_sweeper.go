...
func testSweepHealthcareDicomStore(region string) error {
	resource_name := "HealthcareDicomStore"
	...
	list_template := strings.Split("https://healthcare.googleapis.com/v1beta1/{{dataset}}/dicomStores?dicomStoreId={{name}}", "?")[0]
	...
	resource_list, ok := res["dicomStores"]
	if !ok {
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://healthcare.googleapis.com/v1beta1/{{dataset}}/dicomStores/{{name}}"
