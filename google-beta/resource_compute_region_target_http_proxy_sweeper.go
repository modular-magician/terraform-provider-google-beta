...
func testSweepComputeRegionTargetHttpProxy(region string) error {
	resource_name := "ComputeRegionTargetHttpProxy"
	...
	list_template := strings.Split("https://www.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/targetHttpProxies", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/targetHttpProxies/{{name}}"
