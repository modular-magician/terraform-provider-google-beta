...
func TestAccComputeForwardingRule_mirror(t *testing.T) {
	...
	healthcheckName := fmt.Sprintf("tf-%s", acctest.RandString(10))
	ruleName := fmt.Sprintf("tf-%s", acctest.RandString(10))
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeForwardingRule_mirror(backendName, healthcheckName, ruleName),
			},
...
func testAccComputeForwardingRule_mirror(backendName, healthcheckName, ruleName string) string {
	return fmt.Sprintf(`
...
`, backendName, healthcheckName, ruleName)
}
