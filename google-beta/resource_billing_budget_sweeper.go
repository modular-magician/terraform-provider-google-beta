...
func testSweepBillingBudget(region string) error {
	...
	list_template := strings.Split("https://billingbudgets.googleapis.com/v1beta1/billingAccounts/{{billing_account}}/budgets", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://billingbudgets.googleapis.com/v1beta1/{{name}}"
