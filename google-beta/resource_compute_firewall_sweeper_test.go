...
func testSweepComputeFirewall(region string) error {
	...
	list_template := strings.Split("https://www.googleapis.com/compute/beta/projects/{{project}}/global/firewalls", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/compute/beta/projects/{{project}}/global/firewalls/{{name}}"
