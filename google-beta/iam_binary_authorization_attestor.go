...
func BinaryAuthorizationAttestorIamUpdaterProducer(d *schema.ResourceData, config *Config) (ResourceIamUpdater, error) {
	...
	m, err := getImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/attestors?attestorId=(?P<attestor>[^/]+)/(?P<attestor>[^/]+)", "(?P<project>[^/]+)/(?P<attestor>[^/]+)/(?P<attestor>[^/]+)", "(?P<attestor>[^/]+)/(?P<attestor>[^/]+)"}, d, config, d.Get("attestor").(string))
	if err != nil {
		return nil, err
	}
...
func BinaryAuthorizationAttestorIdParseFunc(d *schema.ResourceData, config *Config) error {
	...
	m, err := getImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/attestors?attestorId=(?P<attestor>[^/]+)/(?P<attestor>[^/]+)", "(?P<project>[^/]+)/(?P<attestor>[^/]+)/(?P<attestor>[^/]+)", "(?P<attestor>[^/]+)/(?P<attestor>[^/]+)"}, d, config, d.Id())
	if err != nil {
		return err
	}
