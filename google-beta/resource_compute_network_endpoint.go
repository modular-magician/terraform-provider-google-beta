...
func resourceComputeNetworkEndpointRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("health_backend_service", flattenComputeNetworkEndpointHealthBackendService(res["healthBackendService"], d)); err != nil {
		return fmt.Errorf("Error reading NetworkEndpoint: %s", err)
	}
	if err := d.Set("health_check", flattenComputeNetworkEndpointHealthCheck(res["healthCheck"], d)); err != nil {
		return fmt.Errorf("Error reading NetworkEndpoint: %s", err)
	}
	if err := d.Set("health_forwarding_rule", flattenComputeNetworkEndpointHealthForwardingRule(res["healthForwardingRule"], d)); err != nil {
		return fmt.Errorf("Error reading NetworkEndpoint: %s", err)
	}
	if err := d.Set("health_state", flattenComputeNetworkEndpointHealthState(res["healthState"], d)); err != nil {
		return fmt.Errorf("Error reading NetworkEndpoint: %s", err)
	}
...
func flattenComputeNetworkEndpointHealthBackendService(v interface{}, d *schema.ResourceData) interface{} {
	return v
}

func flattenComputeNetworkEndpointHealthCheck(v interface{}, d *schema.ResourceData) interface{} {
	return v
}

func flattenComputeNetworkEndpointHealthForwardingRule(v interface{}, d *schema.ResourceData) interface{} {
	return v
}

func flattenComputeNetworkEndpointHealthState(v interface{}, d *schema.ResourceData) interface{} {
	return v
}
...
func flattenNestedComputeNetworkEndpoint(d *schema.ResourceData, meta interface{}, res map[string]interface{}) (map[string]interface{}, error) {
	...
	default:
		return nil, fmt.Errorf("invalid value for items: %v", v)
	}
	...
	for _, vRaw := range items {
		...
		log.Printf("[DEBUG] Checking if item's instance (%#v) is equal to resource's (%#v)", itemInstance, expectedInstance)
		...
		log.Printf("[DEBUG] Checking if item's port (%#v) is equal to resource's (%#v)", itemPort, expectedPort)
		...
		log.Printf("[DEBUG] Checking if item's ipAddress (%#v) is equal to resource's (%#v)", itemIpAddress, expectedIpAddress)
...
func resourceComputeNetworkEndpointDecoder(d *schema.ResourceData, meta interface{}, res map[string]interface{}) (map[string]interface{}, error) {
	...
	for _, item := range items {
		original := item.(map[string]interface{})
		transformed := map[string]interface{}{}

		v, ok := original["networkEndpoint"]
		if !ok || v == nil {
			continue
		}
		...
		v, ok = original["healths"]
		if !ok || v == nil {
			decoded = append(decoded, transformed)
			continue
		}
