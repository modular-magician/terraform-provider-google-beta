...
func testSweepDNSManagedZone(region string) error {
	...
	list_template := strings.Split("https://www.googleapis.com/dns/v1beta2/projects/{{project}}/managedZones", "?")[0]
	...
	for _, ri := range rl {
		...
		if strings.HasPrefix(name, "tf-test") {
			delete_template := "https://www.googleapis.com/dns/v1beta2/projects/{{project}}/managedZones/{{name}}"
