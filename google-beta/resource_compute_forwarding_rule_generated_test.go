...
func TestAccComputeForwardingRule_sharedVPCInternalHttpLbWithMigBackendExample(t *testing.T) {
	t.Parallel()
	...
	vcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeForwardingRule_sharedVPCInternalHttpLbWithMigBackendExample(context),
			},
			{
...
func testAccComputeForwardingRule_sharedVPCInternalHttpLbWithMigBackendExample(context map[string]interface{}) string {
	return Nprintf(`
...
resource "google_compute_region_health_check" "default" {
  ...
  name     = "tf-test-l7-ilb-rhc%{random_suffix}"
  provider = google-beta
  region   = "us-central1"
  http_health_check {
    port_specification = "USE_SERVING_PORT"
